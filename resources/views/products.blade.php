...
<body>
    ...
    <!-- JS مضغوط ومعالج -->
    <script>
        const products=@json($products);const languageData={ar:{mainTitle:"{{ $category->name }}",searchPlaceholder:"ابحث عن منتج بالاسم..."},en:{mainTitle:"{{ $category->name }}",searchPlaceholder:"Search for an item by name..."}};document.addEventListener("DOMContentLoaded",()=>{changePage(1)});function changePage(e){const t=(e-1)*9,n=t+9,o=products.slice(t,n),a=document.getElementById("products-grid");a.innerHTML="";o.forEach(e=>{const t=document.createElement("div");t.className="product-card",t.setAttribute("data-product-id",e.id),t.innerHTML=`<img src="${e.image}" alt="${e.name}" loading="lazy"><h3>${e.name}</h3><p>${e.description}</p><p>${e.price}</p>`,a.appendChild(t)});document.querySelectorAll(".product-card").forEach(e=>{e.addEventListener("click",()=>{window.location.href=`/home/products/details/${e.getAttribute("data-product-id")}`})});document.querySelectorAll("#pagination button").forEach(e=>e.classList.remove("active")),document.querySelectorAll("#pagination button")[e-1].classList.add("active")}function toggleLanguageMenu(){document.getElementById("languageMenu").classList.toggle("show")}function changeLanguage(e){const t=languageData[e];document.getElementById("mainTitle").textContent=t.mainTitle,document.getElementById("searchInput").placeholder=t.searchPlaceholder,toggleLanguageMenu()}function redirectToCustomerService(){window.location.href="/home/compliants"}function searchItem(){const e=document.getElementById("searchInput").value;e?window.location.href=`/home/product_search?keyword=${e}`:alert("Please enter a search term")}document.getElementById("searchInput").addEventListener("keypress",function(e){"Enter"===e.key&&searchItem()});
